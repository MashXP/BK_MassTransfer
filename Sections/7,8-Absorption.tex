...
\begin{itemize}
    ...
    \item \textbf{Desorption or Stripping:} The reverse of absorption, where a solute is transferred from the liquid phase to the gas phase. It is often used to regenerate the solvent. The \textbf{Stripping Factor (S)} is defined as the inverse of the absorption factor: $S = 1/\AbsFactor = (\mEq \Vvar) / \Lvar$.
    ...
    \end{itemize}
    \item \textbf{Solvent Selection:} Ideally, solvents should have low volatility, high solubility for the solute, and low cost. Common examples include water, hydrocarbon oils, and aqueous amines (MEA/DEA) for removing acid gases like $H_2S$ and $CO_2$.
    \item \textbf{Thermal Effects:} Absorption is usually exothermic, leading to an increase in liquid temperature.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Step Off Stages:} Starting from the bottom point $(\xVar_{A,\text{out}}, \yVar_{A,\text{in}})$ on the operating line, draw a horizontal line to the equilibrium curve, then a vertical line back to the operating line. The vertical distance between the operating and equilibrium lines represents the driving force for mass transfer. This completes one theoretical stage.
