...
\begin{itemize}
    \item \textbf{Distillation:} A method to separate liquid solution components based on differences in their volatilities, which governs their distribution between vapor and liquid phases.
    \item \textbf{Equilibrium:} In VLE, the rate of vaporization equals the rate of condensation for each component. The vapor phase is enriched in the more volatile component.
    \item \textbf{Gibbs Phase Rule:} Determines the degrees of freedom (F) for a system at equilibrium.
    \begin{equation}
        F = C - P + 2
    \end{equation}
    where $C$ is the number of components and $P$ is the number of phases. For a binary mixture (C=2) in VLE (P=2), F=2 degrees of freedom exist (e.g., Temperature and Pressure can be fixed).
\end{itemize}
...
\begin{itemize}
    \item \textbf{Definition:} For components A and B (A is more volatile):
    \begin{equation}
        \alphaRel_{AB} = \frac{K_A}{K_B} = \frac{\yVar_A/\xVar_A}{\yVar_B/\xVar_B} > 1
    \end{equation}
    \item \textbf{For Ideal Systems (obeying Raoult's Law):}
    \begin{equation}
        \alphaRel_{AB} = \frac{\Pvap_A}{\Pvap_B}
    \end{equation}
    \item \textbf{Equilibrium Relationship (The Equilibrium Curve):} This equation plots the equilibrium curve on an x-y diagram.
    \begin{equation}
...
\begin{itemize}
    ...
    \item \textbf{x-y Diagram:} Plots vapor mole fraction ($\yVar_A$) vs. liquid mole fraction ($\xVar_A$) at constant pressure. Essential for McCabe-Thiele calculations.
\end{itemize}
...
An unsteady-state process where a charge of liquid is boiled, and the vapor is continuously removed and condensed. Compositions change with time.
\begin{itemize}
    ...
    \item \textbf{With Constant Relative Volatility:} If $\alphaRel$ is constant, the equation can be integrated analytically:
    \begin{equation}
...
Used to separate high-boiling, temperature-sensitive components from non-volatile impurities by co-distilling with an immiscible liquid (water).
\begin{itemize}
...
\begin{itemize}
    ...
    \end{equation}
    Using the reflux ratio $\R = \Lvar/\D$ and $\Vvar = \Lvar + \D$, this becomes:
    \begin{equation}
...
\begin{itemize}
    ...
    \begin{center}
        \begin{tabular}{|l|c|c|}
            \hline
            \textbf{Feed Condition} & \textbf{q Value} & \textbf{Slope ($\q/(\q-1)$)} \\ \hline
            Subcooled Liquid        & $\q > 1$         & Positive ($>1$)                 \\
            Saturated Liquid        & $\q = 1$         & Infinite (Vertical)             \\
            Liquid + Vapor Mix      & $0 < \q < 1$     & Negative                        \\
            Saturated Vapor         & $\q = 0$         & Zero (Horizontal)               \\
            Superheated Vapor       & $\q < 0$         & Positive ($<1$)                 \\ \hline
        \end{tabular}
    \end{center}
\end{itemize}
