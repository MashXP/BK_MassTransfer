...
\begin{itemize}
    ...
    \end{equation}
    \item \textbf{Density ($\rho$):} The density of the wet air mixture is calculated as:
    \begin{equation}
        \rho = \frac{1+\Humidity}{\HumidVolume} \quad (\text{in kg wet air/m}^3)
    \end{equation}
...
\begin{itemize}
    ...
    \end{equation}
    \item \textbf{Calculation Note:} To calculate the final moisture content ($X_2$) when removing a percentage ($P$) of free moisture: calculate initial free moisture $X_{free,1} = X_1 - X^*$, then $X_{free,2} = X_{free,1} \times (1 - P/100)$, and finally $X_2 = X_{free,2} + X^*$.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Falling-Rate Period:} The drying rate decreases as the moisture content falls. This transition point, where the constant rate ends and the falling rate begins, is called the **Critical Moisture Content** (often labeled as point **C** on drying curves). The mechanism of moisture movement within the solid (e.g., diffusion) becomes the limiting factor.
\end{enumerate}
...
\end{equation}
The rate can also be expressed in terms of mass transfer: $\ConstantRate = \frac{\HeatTransferCoeff (\DryBulbTemp - \WetBulbTemp)}{\LatentHeat_w} = k_y M_{av} (\Humidity_w - \Humidity)$.
...
\end{equation}
Where mass velocity is the product of air velocity ($v$) and air density ($\rho$). Note: When calculating $\MassVelocity$ ($kg/h \cdot m^2$) from velocity ($m/s$), multiply by 3600 to convert seconds to hours.